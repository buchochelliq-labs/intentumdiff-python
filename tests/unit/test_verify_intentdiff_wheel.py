...
from scripts.verify_intentdiff_wheel import verify_wheel, verify_wheels
...
def _valid_names() -> list[str]:
    return [
        ...
        "intentdiff-0.0.1b1.dist-info/METADATA",
        "intentdiff-0.0.1b1.dist-info/WHEEL",
    ]


def test_verify_wheel_accepts_native_intentdiff_wheel(tmp_path: Path) -> None:
    wheel = _write_wheel(
        tmp_path / "intentdiff-0.0.1b1-cp312-cp312-win_amd64.whl",
        _valid_names(),
    )
...
def test_verify_wheel_accepts_windows_arm64_wheel(tmp_path: Path) -> None:
    ...
    wheel = _write_wheel(
        tmp_path / "intentdiff-0.0.1b1-cp312-abi3-win_arm64.whl",
        names,
    )
...
def test_verify_wheel_rejects_pure_python_wheel(tmp_path: Path) -> None:
    wheel = _write_wheel(
        tmp_path / "intentdiff-0.0.1b1-py3-none-any.whl",
        _valid_names(),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("intentdiff/../evil.py", "unsafe archive path"),
        ("intentdiff/sitecustomize.py", "sensitive/startup"),
        ("intentdiff/.env", "sensitive/startup"),
        ("intentdiff/payload.pth", "blocked file type"),
        ("intentdiff/scripts/postinstall.ps1", "blocked file type"),
        ("intentdiff/plugins/unexpected.wasm", "Wasm outside"),
        ("intentdiff/extra_native.cp312-win_amd64.pyd", "unexpected native"),
    ],
)
...
def test_verify_wheels_rejects_oversized_release_set(tmp_path: Path) -> None:
    ...
    _write_wheel(
        tmp_path / "intentdiff-0.0.1b1-cp312-cp312-macosx_11_0_arm64.whl",
        _valid_names(),
    )
...
def test_verify_wheel_rejects_wrong_distribution_name(tmp_path: Path) -> None:
    wheel = _write_wheel(
        tmp_path / "notintentdiff-0.0.1b1-cp312-cp312-win_amd64.whl",
        _valid_names(),
    )
...
def test_verify_wheel_rejects_wrong_version(tmp_path: Path) -> None:
    wheel = _write_wheel(
        tmp_path / "intentdiff-0.2.0-cp312-cp312-win_amd64.whl",
        _valid_names(),
    )
...
def test_verify_wheel_rejects_metadata_identity_mismatch(tmp_path: Path) -> None:
    wheel = tmp_path / "intentdiff-0.0.1b1-cp312-cp312-win_amd64.whl"
...
def test_pyproject_uses_top_level_maturin_native_wheel_backend() -> None:
    ...
    assert pyproject["project"]["name"] == "intentdiff"
    ...
    assert pyproject["tool"]["maturin"]["python-packages"] == ["intentdiff"]
...
def _write_wheel_with_provenance(
    ...
    with zipfile.ZipFile(path, "w") as zf:
        zf.writestr("intentdiff/__init__.py", "")
        zf.writestr("intentdiff/intentdiff_rust_core.cp312-win_amd64.pyd", "")
        for name, data in wasm.items():
            zf.writestr(f"intentdiff/wasm/{name}", data)
        if manifest_artifacts is not None:
            zf.writestr(
                "intentdiff/wasm/wasm_provenance.json",
...
def _prov_wheel_name(tmp_path: Path) -> Path:
    return tmp_path / "intentdiff-0.0.1b1-cp312-cp312-win_amd64.whl"
