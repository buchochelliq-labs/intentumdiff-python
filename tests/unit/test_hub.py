...
Unit tests for intentdiff.plugins.hub security features introduced in
...
def _make_spec(**kw):
    from intentdiff.plugins.hub import PluginSpec
...
class TestDepHashesValidation:
    def test_extra_package_without_allowlist_is_rejected(self):
        from intentdiff.plugins.hub import _validate_dep_hashes
...
class TestPipInstall:
    def test_no_dep_hashes_adds_no_deps_flag(self):
        ...
        from intentdiff.plugins.hub import pip_install
    ...
    def test_with_dep_hashes_uses_require_hashes(self, tmp_path):
        ...
        import intentdiff.plugins.hub as _hub
...
class TestInstallTargetClassification:
    def test_pep508_direct_url_is_classified_as_direct_url(self) -> None:
        from intentdiff.plugins.hub import classify_install_target

        assert (
            classify_install_target(
                "intentdiff-plugin @ https://example.test/intentdiff-plugin.whl"
            )
            == "direct_url"
        )
...
class TestLintWheelContents:
    def test_clean_wheel_returns_no_errors(self, tmp_path):
        ...
        from intentdiff.plugins.hub import _lint_wheel_contents
...
class TestPreInstallSecurityCheck:
    ...
    def test_missing_wasm_with_expected_checksums_is_error(self, tmp_path):
        ...
        from intentdiff.plugins.hub import pre_install_security_check
    ...
    def test_dep_hashes_lints_every_dependency_wheel(self, tmp_path):
        ...
        spec = _make_spec(
            ...
            allowed_dependencies=["dep-a"],
            dep_hashes={
                "intentdiff-my-plugin==1.0": _hash("a"),
                "dep-a==2.0": _hash("b"),
            },
        )
        ...
        def fake_download(req_file, dest):
            ...
            _build_wheel(
                ...
                filename="intentdiff_my_plugin-1.0-py3-none-any.whl",
            )
            _build_wheel(
                dest,
                {"dep/__init__.py": b"", "evil.pth": b"/tmp/evil"},
...
class TestPluginSpecDepHashesYaml:
    def test_dep_hashes_round_trip(self, tmp_path):
        ...
        from intentdiff.plugins.hub import PluginSpec, load_plugins_file, save_plugins_file
        ...
        plugins_file = tmp_path / "intentdiff_plugins.yaml"
    ...
    def test_missing_dep_hashes_key_defaults_to_empty(self, tmp_path):
        ...
        pytest.importorskip("yaml", reason="pyyaml not installed")
        from intentdiff.plugins.hub import load_plugins_file
...
class TestValidateRegistryRef:
    ...
    def test_accepts_main(self):
        from intentdiff.plugins.hub import _validate_registry_ref
