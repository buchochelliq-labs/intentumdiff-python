"""Integration tests for the `intentumdiff watch` CLI subcommand (argument parsing only).
...
class TestWatchArgParsing:
    def test_default_paths_is_dot(self) -> None:
        """``intentumdiff watch`` with no positionals defaults to ``['.']``."""
    ...
    def test_func_is_cmd_watch(self) -> None:
        from intentumdiff.cli import _build_parser, _cmd_watch
