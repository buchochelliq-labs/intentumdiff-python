"""Verify that an IntentDiff wheel contains the native Rust core.

The first public package should be a platform wheel, not a pure-Python wheel:
users install ``intentdiff`` once and get the Python API, built-in Wasm plugin
...
PROVENANCE_ENTRY = "intentdiff/wasm/wasm_provenance.json"
...
# file whose name is not `intentdiff_rust_core`, so a stray `.dll` cannot slip in.
NATIVE_SUFFIXES = (".pyd", ".so", ".dylib", ".dll")
REQUIRED_PACKAGES = {
    "intentdiff/__init__.py",
}
NATIVE_MODULE_NAME = "intentdiff_rust_core"
...
def _validate_wheel_entries(
    ...
    *,
    ...
    for name in names:
        ...
        if lower.endswith(".wasm") and not lower.startswith("intentdiff/wasm/"):
            raise ValueError(f"wheel contains Wasm outside intentdiff/wasm: {name!r}")
...
def verify_wheel(
    ...
    *,
    ...
    if "none-any" in wheel.name:
        raise ValueError("IntentDiff release wheel must be native, not py3-none-any")
    ...
    if len(native_modules) != 1:
        raise ValueError(
            "wheel must contain exactly one intentdiff_rust_core native module; "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(summaries) > 1:
        total_size = sum(int(summary["size_bytes"]) for summary in summaries)
        print(
            f"Verified {len(summaries)} IntentDiff release wheels; "
