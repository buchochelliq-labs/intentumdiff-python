...
Configure them in a project-root `intentdiff.yaml`. The policy file is protected by default —
edits to `intentdiff.yaml` itself always produce an immutable guardrail warning. The same file
...
Surfaces: pinned guardrail entries in diffs/reviews, `intentdiff guardrails` in the CLI
