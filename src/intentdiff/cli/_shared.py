...
_NO_BANNER_ENV = "INTENTDIFF_NO_BANNER"
...
def _render_cli_banner(
    *,
    ...
    """Render the IntentDiff banner for interactive CLI sessions."""
    out = console or _console
    title = Text("IntentDiff", style="bold cyan")
    body = Text.assemble(
...
def _version_text() -> str:
    return f"IntentDiff {__version__}"
...
def _run_shell_line(line: str, parser: argparse.ArgumentParser) -> bool:
    ...
    try:
        ...
        return True
    if getattr(args, "command", None) == "shell":
        _err.print("[yellow]Already inside the IntentDiff shell.[/yellow]")
        return True
    try:
        args.no_banner = True
...
def _cmd_shell(args: argparse.Namespace) -> None:
    """Start a lightweight interactive IntentDiff command shell."""
    ...
    from intentdiff.cli._parser import _build_parser

    parser = _build_parser()
    while True:
        try:
            line = input("intentdiff> ")
        except EOFError:
            _console.print()
            return
        except KeyboardInterrupt:
...
def _render_terminal(diff: SemanticDiff) -> None:
    ...
    if not diff.has_semantic_changes:
        ...
        state_message = "IntentDiff found no semantic differences."
        if diff.is_style_only:
            ...
            state_message = "Parse errors were detected, so IntentDiff used token-level fallback."
        _console.print(
            Panel(
                state_message,
...
def _render(diff: SemanticDiff, fmt: str, output: str | None, fuel: int | None = None) -> None:
    ...
    from intentdiff.plugins.loader import load_plugin
...
def _resolve_repo_root(start_path: str | Path) -> Path:
    ...
    from intentdiff.vcs.git_cli import NotAGitRepositoryError, resolve_repo_root
