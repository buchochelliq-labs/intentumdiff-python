"""Tree, markdown-presentation, fallback-diff and stream helpers for intentdiff.differ."""
...
def _compute_structural_hash_for_tree(cst_json: str) -> str:
    ...
    """
    from intentdiff.plugins.loader import _structural_hash_impl
...
def _empty_semantic_tree(language: str) -> SemanticNode:
    digest = hashlib.sha256(f"intentdiff-empty-tree:{language}".encode()).hexdigest()
    return SemanticNode(
        id="0",
...
def _markdown_section_move_presentation(
    ...
    *,
    ...
    from intentdiff.rust_core import try_rust_markdown_section_review
