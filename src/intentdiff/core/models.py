...
class DetectionResult(BaseModel, frozen=True):
    ...
    Returned by :meth:`~intentdiff.SemanticDiffer.detect_language` and
    :meth:`~intentdiff.SemanticDiffer.detect_all`.
    """
...
def _native_rust_core_default() -> bool:
    ...
    IntentDiff ships the first-party Rust core in release wheels, so supported
    native paths should use it automatically.  The legacy
    ``INTENTDIFF_EXPERIMENTAL_RUST_CORE`` env var is still accepted for compatibility,
    while ``INTENTDIFF_RUST_CORE=0`` is the explicit fallback escape hatch.
    """

    raw = os.getenv("INTENTDIFF_RUST_CORE")
    if raw is None:
        raw = os.getenv("INTENTDIFF_EXPERIMENTAL_RUST_CORE")
...
class DiffConfig(BaseModel):
    ...
    profile_phases: bool = Field(
        default_factory=lambda: os.getenv("INTENTDIFF_PROFILE_PHASES", "").strip().lower()
        in {"1", "true", "yes", "on"},
    )
    ...
    # Example CLI usage::
    #
    #   intentdiff index . --lsp python=localhost:2087
