"""
intentdiff.core.indexer
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

High-level indexing engine that walks a repository (or a set of files),
parses each file, and builds a :class:`~intentdiff.core.index.SemanticIndex`.
...
file is processed::
    ...
    from intentdiff.core.indexer import Indexer, IndexProgress
...
Caching
-------
When the :class:`~intentdiff.differ.SemanticDiffer` was created with a
``DiffConfig.cache_path``, two levels of caching apply automatically:

* **Parse-tree cache** — each file's :class:`~intentdiff.core.models.SemanticNode`
...
from intentdiff.core.diffignore import load_diffignore
...
class IndexResult:
    ...
    """The fully built :class:`~intentdiff.core.index.SemanticIndex`."""
...
def _collect_blob_files(
    ...
    by *diffignore*.
    """
    from intentdiff.vcs.git_cli import run_git_bytes  # noqa: PLC0415
...
def _attach_type_info(
    ...
    """
    from intentdiff.core.models import SemanticNode  # noqa: PLC0415
...
class Indexer:
    ...
    :class:`~intentdiff.core.index.SemanticIndex`.

    Parameters
    ----------
    differ:
        A :class:`~intentdiff.differ.SemanticDiffer` instance.  Configure
    ...
    def detect_languages(
        ...
        *on_progress(done, total)* is called after each blob is processed.
        """
        from intentdiff.plugins.exceptions import (  # noqa: PLC0415
            PluginNotFoundError,
        )
        from intentdiff.vcs.git_cli import resolve_repo_root  # noqa: PLC0415
    ...
    def index_repo(
        ...
        *,
        ...
        """
        from intentdiff.vcs.git_cli import (  # noqa: PLC0415
            resolve_repo_root,
            run_git_bytes,
        )
        ...
        if not force and self._differ._cache is not None:
            ...
            if cached is not None:
                ...
                from intentdiff.core.index import SemanticIndex  # noqa: PLC0415
    ...
    async def _index_files_lsp(
        ...
        *,
        ...
        :class:`~intentdiff.lsp.enricher.TypeEnricher`.  After all tasks
        ...
        from intentdiff.core.index import SemanticIndex  # noqa: PLC0415
        from intentdiff.lsp.enricher import TypeEnricher  # noqa: PLC0415
        from intentdiff.plugins.exceptions import PluginNotFoundError  # noqa: PLC0415
