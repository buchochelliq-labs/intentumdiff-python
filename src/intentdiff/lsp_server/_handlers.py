"""
intentdiff.lsp_server._handlers
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    try:
        ...
        if state.supports_codelens_refresh:
            try:
                ...
                log.debug("intentdiff: code lens refresh request failed", exc_info=True)

    except asyncio.CancelledError:
        raise  # propagate cancellation — do not swallow
    except Exception as exc:  # noqa: BLE001
        log.warning("intentdiff: unhandled error during diff for %s: %s", uri, exc)
...
def _do_diff(
    ...
    try:
        ...
        log.warning("intentdiff: diff failed for %s: %s", uri, exc)
        return None
