...
**tcp** — IntentDiff connects to a loopback TCP socket.  Manual
...
:class:`~intentdiff.lsp.client.AsyncLspClient` communicates directly
over those streams.  No TCP socket is involved.

The command templates may contain ``{host}`` and ``{port}`` placeholders
(resolved at launch time by :class:`~intentdiff.lsp.launcher.LspServerProcess`).
...
KNOWN_SERVER_SPECS: dict[str, LspServerSpec] = {
    # ── Stdio-preferred built-ins (intentdiff owns the child pipes) ───────────────
...
and **must** include a ``language`` field so IntentDiff knows which
    language it serves.
    ...
    only be auto-started by :class:`~intentdiff.lsp.launcher.LspServerProcess`
...
treated as already-running and IntentDiff simply connects to
    ``host:port``.
    """
