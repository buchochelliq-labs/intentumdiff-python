"""
intentdiff.plugins.hub
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Plugin hub: resolve, install, verify, and remove IntentDiff plugins.
...
full package in the ``buchochelliq-labs`` GitHub org:
  ``dbt``  ->  ``intentdiff-dbt``
  ->  ``https://github.com/buchochelliq-labs/intentdiff-dbt``
...
1. **Wasm checksum verification** — SHA-256 of every ``.wasm`` file in the
   ...
   ``intentdiff_plugins.yaml`` (populated from the official registry or supplied
...
2. **Wasm capability scan** — the binary's import section is scanned for
   ...
   permitted host import is the IntentDiff WIT package
...
stored in ``intentdiff_plugins.yaml`` on first install (trust-on-first-use), and
subsequent ``intentdiff plugins install`` will verify them.

Plugin file format (intentdiff_plugins.yaml)
----------------------------------------
::

    # intentdiff_plugins.yaml
    version: 1
    plugins:
      - name: dbt
      ...
      - name: custom-plugin
        source: git
        ref: abc1234
        repo: https://github.com/myorg/intentdiff-custom
...
#: GitHub organisation that hosts official IntentDiff plugins.
OFFICIAL_ORG = "buchochelliq-labs"
OFFICIAL_BASE_URL = f"https://github.com/{OFFICIAL_ORG}"

#: Raw-content URL for the official registry manifest (the intentdiff-registry repo — the
#: #95 root of trust, populated at the #82 split; formerly the unpopulated intentdiff-plugins).
OFFICIAL_REGISTRY_RAW_URL = (
    f"https://raw.githubusercontent.com/{OFFICIAL_ORG}/intentdiff-registry"
)
...
_PLUGIN_NAME_RE = re.compile(r"^(?:intentdiff-)?[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")
...
def _validate_registry_ref(ref: str, *, strict: bool = False) -> None:
    ...
    """
    from intentdiff.rust_core import validate_registry_ref as _rust_validate_ref
...
def _validate_dep_hashes(spec: "PluginSpec", install_target: str | None = None) -> list[str]:
    ...
    """
    from intentdiff.rust_core import validate_dep_hashes as _rust_validate_dep_hashes
...
# Any module name NOT in this set (and not an IntentDiff namespace)
...
class PluginSpec:
    ...
    #: Populated automatically by ``intentdiff plugins add``.
    ...
    def __post_init__(self) -> None:
        if not _PLUGIN_NAME_RE.fullmatch(self.name):
            raise ValueError(
                ...
                "IntentDiff plugin name, not a URL, path, or requirement expression."
            )
    ...
    def package_name(self) -> str:
        """Canonical Python package name with the primary ``intentdiff-`` prefix."""
        if self.name.startswith("intentdiff-"):
            return self.name
        return f"intentdiff-{self.name}"

    @property
    def short_name(self) -> str:
        """Short name without the primary package prefix."""
        if self.name.startswith("intentdiff-"):
            return self.name[len("intentdiff-"):]
        return self.name
...
class RegistryEntry:
    ...
    def abi_incompatibility(self, host_version: str | None = None) -> str | None:
        ...
        if not self.abi_target:
            return None
        from intentdiff.plugins.registry_schema import HOST_CONTRACT_VERSION, abi_compatible
        ...
        if not abi_compatible(self.abi_target, host):
            return (
                ...
                f"which is incompatible with this IntentDiff host's contract {host}. "
                "Upgrade IntentDiff or install a plugin build that targets the host's ABI."
            )
        return None
...
def _require_yaml() -> Any:
    ...
    try:
        ...
        raise ImportError(
            "pyyaml is required to read/write intentdiff_plugins.yaml. "
            "Install it with: pip install pyyaml"
        ) from exc


def load_plugins_file(path: Path) -> list[PluginSpec]:
    """Parse *path* (``intentdiff_plugins.yaml``) and return a list of :class:`PluginSpec`."""
    yaml = _require_yaml()
...
def save_plugins_file(path: Path, specs: list[PluginSpec]) -> None:
    """Write *specs* to *path* as a ``intentdiff_plugins.yaml`` file."""
    yaml = _require_yaml()
    ...
    header = (
        "# intentdiff_plugins.yaml - plugin dependencies for IntentDiff\n"
        "# Install all plugins:            intentdiff plugins install\n"
        "# Add/update a plugin:            intentdiff plugins add <name>\n"
        "# Remove a plugin:                intentdiff plugins remove <name>\n"
    ...
    if check_checksums:
        if not spec.wasm_checksums:
            warnings.append(
                ...
                "Use 'intentdiff plugins add' to populate checksums from the official registry."
            )
        else:
