...
File-system watcher for ``intentdiff watch``.  Watches a set of files/directories
for modifications, debounces rapid editor saves, and runs a semantic diff
against the current HEAD for every changed file.

Requires the ``watchdog`` package, which is a core IntentDiff dependency.
"""
...
from intentdiff.sources.git_source import WorkingTreeSource
...
class FileWatcher:
    ...
    render_fn:
        Callable that receives a :class:`~intentdiff.core.models.SemanticDiff`
