...
Windows named pipe (path format ``\\\\.\\pipe\\intentdiff-live-<random>``), which
...
from intentdiff.core.models import ChangeStreamEvent, EditDelta
...
def _wasm_dir() -> str:
    ...
    return str(Path(intentdiff.__file__).parent / "wasm")
...
def _make_socket_dir() -> str:
    ...
    sock_dir = os.path.join(base, f"intentdiff-{os.getpid()}")
...
def _make_socket_path(pid: int, token: str) -> str:
    ...
    return os.path.join(sock_dir, f"intentdiff-live-{token[:16]}.sock")
...
class LiveServer:
    ...
    ----------
    differ:
        A configured :class:`~intentdiff.differ.SemanticDiffer` instance.
    repo_path:
    ...
    def start_socket(self, socket_path: str | None = None) -> str:
        ...
        if _has_unix_socket():
            ...
            pipe_name = r"\\.\pipe\intentdiff-live-" + secrets.token_hex(8)
    ...
    def _run_streaming_review(
        ...
        *,
        ...
        from intentdiff.core.commit_differ import FileDiffError, FileDiffResult
