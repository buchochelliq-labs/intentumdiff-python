...
# fast offline check.  For a live OSV scan run: ``intentumdiff security-check``.
...
_ENFORCE_PROVENANCE_ENV = "INTENTUMDIFF_ENFORCE_WASM_PROVENANCE"
...
def _check_wasmtime_version(
    ...
    *,
    ...
    if override in ("1", "true", "yes"):
        logger.warning(
            "INTENTUMDIFF_ALLOW_VULNERABLE_WASMTIME is set: loading Wasm plugin under "
    ...
    raise PluginLoadError(
        ...
        f"with memory64). Upgrade wasmtime or set INTENTUMDIFF_ALLOW_VULNERABLE_WASMTIME=1 "
        f"to override (not recommended). Run 'intentumdiff security-check' for a live "
...
#
# To force a manual refresh (e.g. from `intentumdiff security-check --refresh`):
...
def query_osv(packages: list[tuple[str, str]]) -> list[dict]:
    ...
    Public so that ``intentumdiff security-check`` can call it directly.
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "intentumdiff/osv-check",
        },
        method="POST",
    )
...
def _log_osv_findings(vulns: list[dict], *, cached: bool) -> None:
    ...
    for v in vulns:
        ...
        logger.warning(
            ...
            "Run 'intentumdiff security-check' for details.",
            tag,
            v["id"],
            pkg_ver,
            v.get("summary", ""),
...
def refresh_osv_cache() -> list[dict]:
    ...
    ``intentumdiff security-check --refresh``.
...
def _check_osv_cache_or_block(
    ...
    *,
    ...
    - Stamp fresh + cache ABSENT: last fetch failed within 24h.  Block until
      the user runs ``intentumdiff security-check --refresh`` or sets the env override.
    """
    ...
    if cache is None:
        ...
        raise PluginLoadError(
            ...
            "undetected vulnerabilities. Run 'intentumdiff security-check --refresh' "
            "to retry, or set INTENTUMDIFF_ALLOW_VULNERABLE_WASMTIME=1 to bypass "
            "(not recommended).",
        )
    ...
    for v in cache:
        if v.get("package") == "wasmtime" and v.get("version") == wasmtime_ver:
            ...
            if override in ("1", "true", "yes"):
                logger.warning(
                    "INTENTUMDIFF_ALLOW_VULNERABLE_WASMTIME: loading plugins despite "
            ...
            raise PluginLoadError(
                ...
                "Upgrade wasmtime, run 'intentumdiff security-check --refresh' for "
                "details, or set INTENTUMDIFF_ALLOW_VULNERABLE_WASMTIME=1 to bypass "
                "(not recommended).",
            )
...
def _maybe_start_osv_check() -> None:
    ...
    with _osv_thread_lock:
        ...
        if age < _OSV_FETCH_INTERVAL:
            ...
            # advisory visibility belongs to `intentumdiff security-check`.
...
_PLUGIN_MEMORY_LIMIT_ENV = "INTENTUMDIFF_PLUGIN_MEMORY_LIMIT_BYTES"
