...
def _cmd_assets_diff(args: argparse.Namespace) -> None:
    from intentumdiff.assets import diff_image_assets
...
def _cmd_assets_git(args: argparse.Namespace) -> None:
    from intentumdiff.assets import diff_git_assets
...
def _cmd_git(args: argparse.Namespace) -> None:
    from intentumdiff.sources.git_source import _REF_STAGED, _REF_UNPUSHED
    ...
    # explicit commit ref is given for --new so that `intentumdiff git --new HEAD`
...
def _cmd_github_pr(args: argparse.Namespace) -> None:
    from intentumdiff.github_app import parse_pull_request_url
    ...
    payload = {
        ...
        "number": ref.number,
        "review_command": (
            f"intentumdiff-github-app review-pr --owner {shlex.quote(ref.owner)} "
...
def _cmd_gist_diff(args: argparse.Namespace) -> None:
    from intentumdiff.github_app import parse_gist_url
    ...
    payload = {
        ...
        "review_command": f"intentumdiff gist-diff {shlex.quote(args.url)} --format html",
    }
    print(json.dumps(payload, indent=2, sort_keys=True))
...
def _cmd_plugins_add(args: argparse.Namespace) -> None:
    ...
    from intentumdiff.plugins.hub import (
        PluginSpec,
        RegistryEntry,
    ...
    if skip_verify:
        ...
        if not yes and not _os.environ.get("INTENTUMDIFF_ALLOW_SKIP_VERIFY"):
            _err.print(
                "[bold red]--skip-verify requires --yes or "
                "INTENTUMDIFF_ALLOW_SKIP_VERIFY=1 to prevent accidental use.[/bold red]"
            )
            sys.exit(1)
        _err.print(
    ...
    if not args.repo and not skip_verify:
        try:
            ...
            short = (
                args.plugin_name[len("intentumdiff-"):]
                if args.plugin_name.startswith("intentumdiff-")
...
def _cmd_plugins_install(args: argparse.Namespace) -> None:
    ...
    if not yaml_path.exists():
        ...
        _err.print(
            "Create one with [bold]intentumdiff plugins add <name>[/bold] "
            "([bold]intentumdiff plugins add <name>[/bold] remains an alias) "
...
def _cmd_security_check(args: argparse.Namespace) -> None:
    ...
    if needs_fetch:
        ...
        try:
            ...
            _err.print(
                ...
                "  [bold]intentumdiff security-check --refresh[/bold]\n"
                "successfully, or set [bold]INTENTUMDIFF_ALLOW_VULNERABLE_WASMTIME=1[/bold] "
    ...
    _console.print(
        ...
        "set [bold]INTENTUMDIFF_ALLOW_VULNERABLE_WASMTIME=1[/bold]."
    )
    sys.exit(1)
...
async def _two_pass_index(
    ...
    from intentumdiff.lsp.servers import KNOWN_SERVER_SPECS, load_lsp_servers_json
...
async def _index_with_progress(
    ...
    *,
    ...
    from intentumdiff.core.indexer import IndexProgress
...
def _cmd_live_server(args: argparse.Namespace) -> None:
    ...
    from intentumdiff.live_server import LiveServer, _write_line
...
def _cmd_lsp_server(args: argparse.Namespace) -> None:
    """Start the IntentumDiff LSP server (stdio or TCP)."""
    try:
        from intentumdiff.lsp_server import create_server
    except ImportError:
        _err.print(
            "[red]Error:[/red] pygls is required for 'lsp-server'.  "
            "Install with: [bold]pip install 'intentumdiff[lsp-server]'[/bold]"
        )
        sys.exit(1)
    ...
    if tcp_port is not None:
        _err.print(
            f"[bold green]IntentumDiff LSP[/bold green] listening on "
...
def _cmd_serve(args: argparse.Namespace) -> None:
    """Start the IntentumDiff HTTP playground server."""
    try:
        ...
        _err.print(
            ...
            "Install with: [bold]pip install 'intentumdiff[serve]'[/bold]"
        )
        sys.exit(1)
    ...
    _console.print(
        f"[bold green]IntentumDiff serve[/bold green] listening on "
...
if TYPE_CHECKING:
    from intentumdiff.lsp.config import LspServerConfig as LspServerConfig  # noqa: F811
...
def _add_diagnostics_db_arg(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        ...
        help=(
            ...
            "Use intentumdiff diagnostics summary/query to inspect it."
        ),
    )
...
def _add_profile_phases_arg(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        ...
        help=(
            ...
            "to diff results. Also enabled by INTENTUMDIFF_PROFILE_PHASES=1."
        ),
    )
...
def _add_guardrail_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Use a specific intentumdiff.yaml policy file for protected config "
            "guardrail rules."
        ),
    )
    p.add_argument(
