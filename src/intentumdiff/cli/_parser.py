...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Suppress the interactive IntentumDiff banner.",
    )
    ...
    shell_p = sub.add_parser(
        "shell",
        help="Start the interactive IntentumDiff command shell.",
        description=(
            "Start a lightweight IntentumDiff shell. Commands entered at the "
    ...
    shell_p.add_argument(
        ...
        help="Start the shell without the IntentumDiff banner.",
    )
    ...
    github_pr_p = sub.add_parser(
        "github-pr",
        help="Parse a GitHub pull request URL into an IntentumDiff review target.",
    )
    ...
    gist_diff_p = sub.add_parser(
        "gist-diff",
        help="Parse a GitHub Gist URL into an IntentumDiff diff target.",
    )
    ...
    check_p.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Use a specific intentumdiff.yaml policy file.",
    )
    check_p.add_argument(
        "--strict",
        action="store_true",
    ...
    plugins_p.set_defaults(func=_cmd_plugins_list, file="intentumdiff_plugins.yaml")
    ...
    pl_add.add_argument(
        ...
        help="Short name (e.g. dbt) or full package name (intentumdiff-dbt).",
    )
    pl_add.add_argument(
    ...
    pl_add.add_argument(
        ...
        help="Custom git repo URL. Omit to use the official intentumdiff registry org.",
    )
    pl_add.add_argument(
        "--file", default="intentumdiff_plugins.yaml", metavar="FILE",
        help="Plugins manifest to update (default: intentumdiff_plugins.yaml).",
    )
    pl_add.add_argument(
        "--no-save", dest="save", action="store_false",
        help="Install without updating intentumdiff_plugins.yaml.",
    )
    pl_add.add_argument(
    ...
    pl_install = plugins_sub.add_parser(
        "install",
        help="Install all plugins declared in intentumdiff_plugins.yaml.",
    )
    pl_install.add_argument(
        "--file", default="intentumdiff_plugins.yaml", metavar="FILE",
        help="Plugins manifest to read (default: intentumdiff_plugins.yaml).",
    )
    pl_install.add_argument(
    ...
    pl_remove = plugins_sub.add_parser(
        "remove",
        help="Uninstall a plugin and remove it from intentumdiff_plugins.yaml.",
    )
    ...
    pl_remove.add_argument(
        "--no-save", dest="save", action="store_false",
        help="Uninstall without updating intentumdiff_plugins.yaml.",
    )
    ...
    index_p.add_argument(
        "--cache-path",
        default=".intentumdiff-cache",
        metavar="PATH",
        dest="cache_path",
        help="Cache directory (default: .intentumdiff-cache inside the repo).",
    )
    index_p.add_argument(
        "--force",
        action="store_true",
    ...
    cache_p.set_defaults(cache_path=".intentumdiff-cache")
    ...
    _cache_path_arg = dict(
        ...
        default=".intentumdiff-cache",
        metavar="PATH",
        help="Cache directory (default: .intentumdiff-cache).",
    )
    ...
    _diagnostics_db_arg = dict(
        dest="db",
        default=".intentumdiff/diagnostics.duckdb",
        metavar="FILE",
        help="Diagnostics DuckDB path (default: .intentumdiff/diagnostics.duckdb).",
    )
    ...
    live_p = sub.add_parser(
        ...
        description=(
            "Run IntentumDiff as a persistent JSON LiveServer endpoint. "
...
if _CLICK_RUNTIME_AVAILABLE:
    ...
    @click.version_option(__version__, "--version", message="IntentumDiff %(version)s")
    @click.option(
        "--no-banner",
        is_flag=True,
        default=False,
    ...
    for _name, _help in [
        ("shell", "Start the interactive IntentumDiff command shell."),
