...
_CANONICAL_PYTHON_PLUGIN_ID = "intentumdiff:python:python"
...
def _pyo3_extension_module_name() -> str | None:
    ...
    NOT executing anything in the PyO3-free cdylib wheel — where ``intentumdiff.intentumdiff_rust_core``
    ...
    # Cover both layouts: the .pyd bundled inside the `intentumdiff` package (release pyo3 wheel:
    # `intentumdiff.intentumdiff_rust_core`) and the standalone maturin package whose extension is the
    # like-named submodule (dev: `intentumdiff_rust_core.intentumdiff_rust_core`).
    for module_name in (
        "intentumdiff.intentumdiff_rust_core",
        "intentumdiff_rust_core.intentumdiff_rust_core",
...
# The compiled core exports the language-agnostic C ABI (`intentumdiff_call`/`intentumdiff_free`,
...
def _extension_path() -> str:
    """Filesystem path to the compiled core that exports the C ABI (`intentumdiff_call`).
    ...
    (``intentumdiff/intentumdiff_rust_core/intentumdiff_rust_core.{dll,so,dylib}``). Uses ``find_spec``
    ...
    if name is not None:
        ...
        if spec is not None and spec.origin:
            return spec.origin
    # 2. The bare cdylib: search the intentumdiff_rust_core subpackage dir (and the intentumdiff package
    #    dir) for `*intentumdiff_rust_core*.{dll,so,dylib}`, via find_spec locations.
    search_dirs: list[str] = []
    for package_name in ("intentumdiff.intentumdiff_rust_core", "intentumdiff_rust_core", "intentumdiff"):
        try:
    ...
    for directory in search_dirs:
        ...
        for entry in entries:
            if "intentumdiff_rust_core" in entry and (
                entry.endswith(lib_suffixes) or entry.endswith(ext_suffixes)
            ):
                return os.path.join(directory, entry)
    raise RuntimeError("intentumdiff_rust_core compiled core not found for the C ABI")
...
def _c_abi_lib() -> Any:
    ...
    if _C_ABI_LIB is None:
        ...
        lib.intentumdiff_call.argtypes = [ctypes.c_char_p, ctypes.c_char_p]
        lib.intentumdiff_call.restype = ctypes.c_void_p
        lib.intentumdiff_free.argtypes = [ctypes.c_void_p]
        lib.intentumdiff_free.restype = None
        _C_ABI_LIB = lib
    return _C_ABI_LIB
...
def _c_abi_call(name: str, *args: Any) -> Any:
    ...
    ptr = lib.intentumdiff_call(
        name.encode("utf-8"), json.dumps(payload).encode("utf-8")
    )
    if not ptr:
        raise RuntimeError(f"intentumdiff_call({name!r}) returned NULL (allocation failure)")
    try:
        raw = ctypes.cast(ptr, ctypes.c_char_p).value
    finally:
        lib.intentumdiff_free(ptr)
...
def load_project_diff_config_data(
    ...
    """Resolve the ``intentumdiff.yaml`` ``config`` section in the Rust core (#99, A2.1).
...
def resolve_intentumdiff_config_path(
    start_path: str | None, explicit_path: str | None
) -> str | None:
    """Find the nearest ``intentumdiff.yaml`` in the Rust core (#99, A2.1)."""
    backend = _load_backend()
    find_fn = getattr(backend, "find_intentumdiff_config_path", None)
    if not callable(find_fn):
        raise RuntimeError("backend does not expose find_intentumdiff_config_path")
...
def try_rust_core_commit_json(
    *,
    ...
    try:
        ...
        _record_adapter_phase("rust_core_commit_control_validation", phase_started)
        if os.getenv("INTENTUMDIFF_RUST_CORE_VALIDATE_JSON", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
